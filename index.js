...
async function getUserStats(userId) {
  const result = await pool.query(
    'SELECT total_exchanged FROM user_stats WHERE user_id = $1',
    [userId]
  );
...
async function getAllUserStats() {
  const result = await pool.query(
    'SELECT user_id, total_exchanged FROM user_stats ORDER BY total_exchanged DESC'
  );
  return result.rows;
}
...
async function getTotalExchanged() {
  const result = await pool.query(
    'SELECT total_exchanged FROM global_stats WHERE id = 1'
  );
  return parseFloat(result.rows[0]?.total_exchanged || 0);
}

async function updateTotalExchanged(amount) {
  await pool.query(
    'UPDATE global_stats SET total_exchanged = total_exchanged + $1 WHERE id = 1',
    [amount]
  );
}
...
const commands = [
  ...
  {
    name: 'set',
    description: 'Set user stats manually (Admin only)',
    default_member_permissions: PermissionFlagsBits.Administrator.toString(),
    options: [
      {
        name: 'user',
        type: 6,
        description: 'User to set stats for',
        required: true,
      },
      {
        name: 'amount',
        type: 10,
        description: 'Total amount to set for this user',
        required: true,
      },
    ],
  },
...
client.once('ready', async () => {
  ...
  try {
    ...
    if (guild) {
      ...
      console.log(`📂 Loaded from database: stats=${statsChannelId}, leaderboard=${leaderboardChannelId}, history=${historyChannelId}`);
      ...
      console.log(`✅ Channel IDs saved: stats=${statsChannelId}, leaderboard=${leaderboardChannelId}, history=${historyChannelId}`);
...
client.on('interactionCreate', async (interaction) => {
  try {
    ...
    if (interaction.isStringSelectMenu()) {
      if (interaction.customId === 'exchange_select_v4' || interaction.customId === 'exchange_select_v3' || interaction.customId === 'exchange_select' || interaction.customId === 'exchange_select_v2') {
        await handleExchangeSelection(interaction);
      } else if (interaction.customId === 'crypto_select_v4' || interaction.customId === 'crypto_select_v3' || interaction.customId === 'crypto_select' || interaction.customId === 'crypto_select_v2') {
        await handleCryptoSelection(interaction);
      } else if (interaction.customId === 'crypto_send_select_v4' || interaction.customId === 'crypto_send_select_v3' || interaction.customId === 'crypto_send_select' || interaction.customId === 'crypto_send_select_v2') {
        await handleCryptoSendSelection(interaction);
      } else if (interaction.customId === 'crypto_receive_select_v4' || interaction.customId === 'crypto_receive_select_v3' || interaction.customId === 'crypto_receive_select' || interaction.customId === 'crypto_receive_select_v2') {
        await handleCryptoReceiveSelection(interaction);
      } else if (interaction.customId === 'network_select_v4' || interaction.customId === 'network_select_v3' || interaction.customId === 'network_select' || interaction.customId === 'network_select_v2') {
        await handleNetworkSelection(interaction);
      } else if (interaction.customId === 'network_send_select_v4' || interaction.customId === 'network_send_select_v3' || interaction.customId === 'network_send_select' || interaction.customId === 'network_send_select_v2') {
    ...
    if (commandName === 'set') {
      ...
      await interaction.editReply({ content: `✅ Set ${user.username}'s stats to $${amount.toFixed(2)}!` });
      return;
    }
    ...
    if (commandName === 'price') {
      ...
      if (!CRYPTO_IDS[crypto]) {
        return interaction.editReply(`❌ Invalid cryptocurrency: ${crypto}. Use /supported to see available cryptocurrencies.`);
      }

      if (!FIAT_CURRENCIES.includes(currency)) {
        return interaction.editReply(`❌ Invalid fiat currency: ${currency}. Use /supported to see available currencies.`);
      }
...
async function handleExchangePanel(interaction) {
  try {
    ...
    await interaction.reply({ content: '❌ Failed to create exchange panel. Please try again.', ephemeral: true }).catch(() => {});
  }
}
...
async function handleExchangeSelection(interaction) {
  ...
  if (validTickets.length !== userTickets.length) {
    console.log(`🧹 Pruned ${userTickets.length - validTickets.length} deleted channel(s) for user ${interaction.user.id}`);
...
async function handleNetworkSendSelection(interaction) {
  ...
  const embed = new EmbedBuilder()
    .setTitle('Select What to Receive')
    .setDescription(`**Sending:** ${pendingData.cryptoSendingLabel} (${selectedNetwork.label})\n\nNow select what you want to receive:`)
    .setColor('#5865F2');
...
async function handleCryptoReceiveSelection(interaction) {
  ...
  // Show modal asking for amount
  const modal = new ModalBuilder()
    .setCustomId('amount_modal_crypto')
    .setTitle('Enter Amount');
...
async function handleAmountModal(interaction) {
  ...
  if (pendingData.feeInfo) {
    ...
    if (rangeMatch) {
      feePercent = parseFloat(rangeMatch[2]); // Use the higher percentage
    } else {
      const match = pendingData.feeInfo.match(/(\d+)%/);
      if (match) {
        feePercent = parseFloat(match[1]);
      }
    }
...
async function handleConfirmExchange(interaction) {
  ...
  try {
    ...
    ticketDescription += `**Fee:** ${pendingData.feePercent}% (min $5)\n\n` +
...
async function handleCompleteTicket(interaction) {
  ...
  try {
    ...
    for (const [uid, tickets] of activeTickets.entries()) {
      console.log(`🔎 Checking user ${uid} tickets:`, tickets.map(t => t.channelId));
    ...
    if (ticketOwnerId) {
      ...
      console.log(`📈 User ${ticketOwnerId} stats updated: $${parseFloat(currentUserStats).toFixed(2)} → $${parseFloat(newUserStats).toFixed(2)}`);
    } else {
    ...
    const ticketOwnerUser = ticketOwnerId ? await interaction.guild.members.fetch(ticketOwnerId).catch(() => null) : null;
    ...
    const completeEmbed = new EmbedBuilder()
      ...
      .setDescription(
        ...
        (ticketOwnerId ? `**User Stats Updated:** ✅ (+$${amount.toFixed(2)})` : `**User Stats Updated:** ⚠️ (Owner not found)`) + `\n\n` +
    ...
    if (historyChannelId) {
      try {
        const historyChannel = await interaction.guild.channels.fetch(historyChannelId);
        if (historyChannel) {
          const historyEmbed = new EmbedBuilder()
            .setTitle('Exchange Complete')
            .setDescription(
              `A client successfully exchanged $${amount.toFixed(2)} ${fromMethod} to $${amount.toFixed(2)} ${toMethod} through <@${client.user.id}>!`
            )
            .setColor('#5865F2')
            .setFooter({ text: 'Client hidden for privacy purposes' })
            .setTimestamp();

          await historyChannel.send({ embeds: [historyEmbed] });
          console.log('📜 Posted transaction to history channel');
        }
      } catch (err) {
        console.error('Error posting to history channel:', err);
      }
    }
...
async function handleCloseTicket(interaction) {
  ...
  console.log(`🔍 Close-ticket channel check: name="${interaction.channel.name}", hasParent=${!!interaction.channel.parent}, parentName="${interaction.channel.parent?.name}"`);
...
async function updateLeaderboard(guild) {
  ...
  try {
    ...
    if (sortedUsers.length === 0) {
      ...
      description = sortedUsers.map(({ user_id, total_exchanged }, index) => {
        const medal = index === 0 ? '🥇' : index === 1 ? '🥈' : index === 2 ? '🥉' : `**${index + 1}.**`;
        return `${medal} <@${user_id}> - **$${parseFloat(total_exchanged).toFixed(2)}**`;
      }).join('\n');
    }
...
async function updateStatsChannel(guild) {
  ...
  try {
    ...
    if (error.code === 50013) {
      ...
      console.log('⏱️ Stats channel update rate-limited by Discord (2 changes per 10min). Will retry on next scheduled update.');
    } else {
